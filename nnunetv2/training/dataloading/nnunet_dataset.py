...
from batchgenerators.utilities.file_and_folder_operations import join, load_json, load_pickle, isfile, write_pickle, subfiles
...
class nnUNetDatasetBlosc2CLS(nnUNetDatasetBlosc2):
    def __init__(self, folder: str, identifiers: List[str] = None,
        ...
        super().__init__(folder, identifiers, folder_with_segs_from_previous_stage)
        blosc2.set_nthreads(1)
        dataset_name = os.path.basename(os.path.dirname(self.source_folder))
        raw_dataset_folder = join(nnUNet_raw, dataset_name)
        dataset_json = load_json(join(raw_dataset_folder, 'dataset.json'))
        cls_filename = dataset_json.get('classification_labels_file', 'cls_data.csv')
        cls_path = join(raw_dataset_folder, cls_filename)
        dataframe = pd.read_csv(cls_path)
        identifier_column = next(
            (name for name in ('case_id', 'identifier', 'case', 'id') if name in dataframe.columns),
            dataframe.columns[0],
        )
        label_column = os.environ.get('AUTOMSC_CLS_LABEL_COLUMN', 'label')
        if label_column not in dataframe.columns:
            raise KeyError(
                f'{cls_path} has no classification column {label_column!r}. '
                'Set AUTOMSC_CLS_LABEL_COLUMN when the CSV has multiple targets.'
            )
        self.dataframe = dataframe[[identifier_column, label_column]].rename(
            columns={identifier_column: 'identifier', label_column: 'label'}
        )
