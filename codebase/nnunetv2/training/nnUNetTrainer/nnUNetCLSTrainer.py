...
class nnUNetCLSTrainerMTL(nnUNetTrainer):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.cls_class_num = self.get_cls_class_num(self.get_cls_csv_path())
    ...
    def get_cls_class_num(self, df_path: str) -> int:
        ...
        if label_column not in df.columns:
            raise KeyError(
                f'{df_path} has no classification column {label_column!r}. '
                'Set AUTOMSC_CLS_LABEL_COLUMN when the CSV has multiple targets.'
            )
        return df[label_column].nunique()

    def get_cls_csv_path(self) -> str:
        dataset_name = os.path.basename(self.preprocessed_dataset_folder_base)
        ...
        return join(raw_dataset_folder, cls_filename)
    @staticmethod
